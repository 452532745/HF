       ...
       <div class="wrap">
         ...
         <div class="img-box">
           ...
           <transition-group tag="ul" class="img-list" name="list" mode="in-out">
             <li v-for="(item,index) in selfData[type][page][product].src" :key="item" v-show="index === mark" @mouseenter="stop" @mouseleave="go">
         ...
         <ul class="feature">
           <li v-for="(item,index) in selfData[type][page][product].li" :key="item"><span class="dash"></span>{{ item }}</li>
         </ul>
       </div>
     </div>
